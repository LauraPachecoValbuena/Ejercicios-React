...
//esta interfaz de propiedades. Es la función q hemos creado en el padre (app) xa guardar el token.
interface IProps {
...
const Login: React.FC<IProps> = props => {
  ...
  const getToken = () => {
    fetch("http://localhost:3000/api/auth", {
      ...
      if (res.ok) {
        res.text().then(token => {
          props.saveToken(token); //aqui estoy llamando a la función q guarda el token.
        });
      }
    });
  };
